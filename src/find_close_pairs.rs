use crate::planes::{self, Plane};
use crate::points::{self, Point, PointRef, PointSet};
use std::hash::Hash;
use std::rc::Rc;
...
pub struct State {
    ...
    pub miss_altitude: u32,
    pub current_time: u32,
    pub contents: HashMap<String, (u32,PointsResult)>,
    planes: Vec<Plane>,
}
impl State {
    pub fn new(distance: f32, altitude: u32, planes: Vec<Plane>) -> Self {
        Self {
    ...
    pub fn try_push_points(&mut self, p1: Rc<PointRef>, p2: Rc<PointRef>) {
        ...
        let value = PointsResult::new(Point { x: p1.point.x, y: p1.point.y }, Point { x: p1.point.x, y: p2.point.y });
...
impl ToString for State {
    fn to_string(&self) -> String {
        ...
        for (key, value) in &self.contents {
            res.push_str(&format!("planes {} have a near miss: ({}, {}) - ({}, {})\n", key, value.1.p1.x, value.1.p1.y, value.1.p2.x, value.1.p2.y));
        }
        res
    }
}
